...
void printMatrix()
{
  ...
  for(row = 0; row < maxrow; row++)
  {
    ...
    {
      printf("%d\t",matrix[0][row*maxcol +col]);
    }
    printf("|\n");
  }
  printf("-------------------------------------------------------------------\n");
...
{
  int row, col;
  matrix = (int**)calloc(1,sizeof(int*));
  matrix[0] = (int*)calloc(maxrow*maxcol,sizeof(int));
  for(row = 0; row < maxrow; row++)
  {
    for(col = 0; col < maxcol; col++)
    {
    ...
    {/*
      for(row = 0; row < maxrow; row++)
      {
        for(col = 0; col < maxcol; col++)
        {
          ...
          {
            int value = matrix[0][row*maxcol +col] + ((row + col) * alteration);
            matrix[0][row*maxcol +col] = value;
          }
          else if(currentProcessor == rank)
          {
            int value = matrix[0][row*maxcol +col] + (row + col) * alteration;
            MPI_Win_lock(MPI_LOCK_SHARED,0,0,win);
            MPI_Put(&(value), 1, MPI_INT, 0, col+row*maxcol, 1, MPI_INT, win); 
    ...
    else //Second operation
    {
      ...
      for(row = 0; row < maxrow; row++)
      {
        for(col = 0; col < maxcol; col++)
        {
          if(currentProcessor == rank && rank == 0)
          {
            matrix[0][row*maxcol +col] = matrix[0][row*maxcol +col] + (row + col) * alteration;
          }
          else if(currentProcessor == rank)
          {
            int value = (col == 0) ? matrix[0][row*maxcol +col] + row*alteration : matrix[0][row*maxcol +col] + matrix[0][col-1]*alteration;
            MPI_Win_lock(MPI_LOCK_SHARED,0,0,win);
            MPI_Put(&(value), 1, MPI_INT, 0, col+row*maxcol, 1, MPI_INT, win); 
            MPI_Win_unlock(0,win);
          }
